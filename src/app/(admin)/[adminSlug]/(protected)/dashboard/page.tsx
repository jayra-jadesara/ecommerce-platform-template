...
function canDashboardCard(
  admin: AdminContext,
  permission: Permission,
): boolean {
  const cap = ROLE_UI_CAPABILITIES.find((c) => c.permission === permission);
  if (!cap) return hasPermission(admin, permission);
  return hasUiCapability(admin.permissions, cap);
}
...
export default async function AdminDashboardPage({
  ...
  const canCharts = canDashboardCard(admin, "dash_charts.view");
  ...
  return (
    <div className={adminPageStack()}>
      ...
      {canAttention || setupIncomplete ? (
        <div
          className={cn(
            "grid gap-6",
            setupIncomplete && canAttention ? "lg:grid-cols-[1.15fr_0.85fr]" : "",
          )}
        >
          {canAttention ? (
