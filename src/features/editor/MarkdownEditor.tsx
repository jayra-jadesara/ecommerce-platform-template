...
} from "@/features/editor/markdown-editor";
import { MarkdownContent } from "@/features/editor/markdown";
...
export function MarkdownEditor({
  ...
  previewEmptyHint = "Nothing to preview yet. Switch to Write and add content.",
  footerHint = "You can add headings, lists, links, and images.",
  rows = 18,
}: MarkdownEditorProps) {
        ...
        >
          {TOOLS.map((tool) => (
            <button
              key={tool.id}
              type="button"
              disabled={disabled || (tool.id === "image" && !onRequestImage)}
              title={tool.label}
      ...
      {tab === "write" ? (
        ...
        <div className="min-h-[18rem] px-4 py-4">
          {value.trim() ? (
            ...
            <p className="text-sm text-[var(--color-muted)]">{previewEmptyHint}</p>
          )}
        </div>
      )}
